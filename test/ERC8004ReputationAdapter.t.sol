...
contract ERC8004ReputationAdapterTest is Test {
    ...
    string[] public allSealTypes = [
        "SKILLFUL", "RELIABLE", "THOROUGH", "ENGAGED", "HELPFUL", "CURIOUS",
        "FAIR", "ACCURATE", "RESPONSIVE", "ETHICAL", "CREATIVE", "PROFESSIONAL", "FRIENDLY"
    ];
    ...
    function test_giveFeedback_ScoreConversion_Clamp() public {
        ...
        uint256 sealId1 = adapter.giveFeedback(
            AGENT1_ID, -50, 0, "SKILLFUL", "H2A", "", "", EVIDENCE_HASH
        );
        assertEq(sealRegistry.getSeal(sealId1).score, 0);
        
        // Test value > 100 clamped to 100
        uint256 sealId2 = adapter.giveFeedback(
            AGENT1_ID, 150, 0, "RELIABLE", "H2A", "", "", EVIDENCE_HASH
        );
        assertEq(sealRegistry.getSeal(sealId2).score, 100);
        
        // Test normal value preserved
        uint256 sealId3 = adapter.giveFeedback(
            AGENT1_ID, 85, 0, "THOROUGH", "H2A", "", "", EVIDENCE_HASH
        );
    ...
    function test_isValidTag1_AllSealTypes() public {
        ...
        for (uint i = 0; i < 13; i++) {
            assertTrue(adapter.isValidTag1(allSealTypes[i]), 
                string.concat("Should be valid: ", allSealTypes[i]));
        }
    ...
    function test_isValidTag2_AllQuadrants() public {
        ...
        for (uint i = 0; i < 4; i++) {
            assertTrue(adapter.isValidTag2(allQuadrants[i]), 
                string.concat("Should be valid: ", allQuadrants[i]));
        }
    ...
    function test_giveFeedback_EmitsEvent() public {
        ...
        emit IERC8004ReputationAdapter.FeedbackGiven(
            AGENT1_ID, human1, 85, "SKILLFUL", "H2A", 1 // sealId will be 1
        );
        
        adapter.giveFeedback(
            AGENT1_ID, 85, 0, "SKILLFUL", "H2A", "", "", EVIDENCE_HASH
        );
    ...
    function test_giveFeedback_RevertOn_InvalidTag1() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(
            IERC8004ReputationAdapter.InvalidTag1.selector, "INVALID_SEAL"
        ));
        adapter.giveFeedback(
            AGENT1_ID, 85, 0, "INVALID_SEAL", "H2A", "", "", EVIDENCE_HASH
        );
    ...
    function test_giveFeedback_RevertOn_InvalidTag2() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(
            IERC8004ReputationAdapter.InvalidTag2.selector, "INVALID_QUAD"
        ));
        adapter.giveFeedback(
            AGENT1_ID, 85, 0, "SKILLFUL", "INVALID_QUAD", "", "", EVIDENCE_HASH
        );
    ...
    function test_giveFeedback_RevertOn_SelfFeedback() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(
            IERC8004ReputationAdapter.SelfFeedbackNotAllowed.selector, agent1
        ));
        adapter.giveFeedback(
            AGENT1_ID, 85, 0, "SKILLFUL", "H2A", "", "", EVIDENCE_HASH
        );
    ...
    function test_giveFeedback_RevertOn_NonexistentAgent() public {
        ...
        adapter.giveFeedback(
            ...
            85, 0, "SKILLFUL", "H2A", "", "", EVIDENCE_HASH
        );
    ...
    function test_roundTrip_PreservesData() public {
        ...
        IERC8004ReputationAdapter.ERC8004Feedback memory feedback = 
            adapter.getFeedbackAsERC8004(sealId);
    ...
    function test_adapter_WorksWithExistingSealRegistry() public {
        ...
        vm.startPrank(human1);
        uint256 directSealId = sealRegistry.issueSealH2A(
            AGENT1_ID, sealRegistry.RELIABLE(), 90, keccak256("direct")
        );
        vm.stopPrank();
        
        // Issue a seal through adapter
        vm.startPrank(human2);
        uint256 adapterSealId = adapter.giveFeedback(
            AGENT1_ID, 80, 0, "SKILLFUL", "H2A", "", "", keccak256("adapter")
        );
        vm.stopPrank();
    ...
    function test_giveFeedback_EdgeCases() public {
        ...
        uint256 sealId1 = adapter.giveFeedback(
            AGENT1_ID, 0, 0, "SKILLFUL", "H2A", "", "", bytes32(0)
        );
        assertEq(sealRegistry.getSeal(sealId1).score, 0);
        
        // Test with max value
        uint256 sealId2 = adapter.giveFeedback(
            AGENT1_ID, 100, 0, "RELIABLE", "H2A", "", "", bytes32(0)
        );
        assertEq(sealRegistry.getSeal(sealId2).score, 100);
        
        // Test with empty strings
        uint256 sealId3 = adapter.giveFeedback(
            AGENT1_ID, 50, 0, "THOROUGH", "H2A", "", "", bytes32(0)
        );
        IERC8004ReputationAdapter.ERC8004Feedback memory feedback = 
            adapter.getFeedbackAsERC8004(sealId3);
        assertEq(feedback.endpoint, "");
        assertEq(feedback.feedbackURI, "");
    ...
    function test_comprehensive_TagMappings() public {
        ...
        for (uint i = 0; i < 13; i++) {
            ...
            uint256 sealId = adapter.giveFeedback(
                AGENT1_ID, int128(uint128(50 + i)), 0, sealTypeName, "H2A", "", "", EVIDENCE_HASH
            );
            
            SealRegistry.Seal memory seal = sealRegistry.getSeal(sealId);
            bytes32 expectedSealType = keccak256(bytes(sealTypeName));
            
            assertEq(seal.sealType, expectedSealType, 
                string.concat("Seal type mismatch for ", sealTypeName));
